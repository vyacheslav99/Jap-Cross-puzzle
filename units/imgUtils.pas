...
procedure SortColors(Colors: TColors);
...
procedure SortColors(Colors: TColors);
  ...
  procedure _sort(arr: TColors; depth: byte);
  var
    k, len, i, j: integer;
    c: TColor;
    hc1, hc2: THLSColor;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        hc1 := RGBtoHLS(arr[j]);
        hc2 := RGBtoHLS(arr[j + k]);

        while (j >= 0) and (
          ((depth = 1) and (hc1.H < hc2.H)) or
          ((depth = 2) and (hc1.H = hc2.H) and (hc1.L < hc2.L)) or
          ((depth = 3) and (hc1.H = hc2.H) and (hc1.L = hc2.L) and (hc1.S < hc2.S)) ) do
        begin
          c := arr[j];
          arr[j] := arr[j + k];
          arr[j + k] := c;
...
begin
  _sort(Colors, 1); // по H
  _sort(Colors, 2); // по L внутри групп по H
  _sort(Colors, 3); // по S внутри групп по H и L
end;
